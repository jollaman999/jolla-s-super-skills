...
#
# 남의 repo(클론해 둔 남의 프로젝트)에서 실행하면 그 repo 의 관례가 나온다.
...
# --- 이 repo 가 내 것인가 ---
# shortlog 는 전체 이력을 한 번에 집계한다. 저자별로 git log 를 실행하면 매칭 안 되는
...
| 내 repo 인데 다른 계정으로 커밋함 | \`PROFILE_ME=\"이름,메일\"\` 을 주고 다시 실행한다 |
