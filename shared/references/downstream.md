...
| **사용자가 대상을 손으로 지목함** | 내가 못 알아낸 것이다. 그 자리에서 기록에 넣을지 묻는다 |
| 후보 없음 | 아무 말도 하지 않는다. 없는 걸 있는 것처럼 만들지 않는다. |

**사용자가 "저기도 반영해" 라고 말했으면 기록이 없거나 낡은 것이다.** 한 프로젝트에서만 몇 주 사이
같은 지시를 여섯 번 받았다 (terraform repo 와 현장 패치용 폴더). 그때마다 시키는 대로 하고 끝내면
다음에 또 시킨다. **시킨 일을 한 뒤에 그 대상을 기록에 넣을지 묻는다.**

**`## 링크로 연결된 곳` 은 후보가 아니라 확정이다.** 다른 절은 글자를 찾은 것이라 산문일 수
있지만, 이 절은 링크를 따라가서 이 repo 안에 닿은 것만 낸다. 진짜인지 묻지 말고 **바뀐 파일이
그 링크 대상에 들어 있는지만 보고 보고한다.** `.git/hooks` 에 깔린 커밋 훅이 대표적이다.
...
| 설정을 고쳤을 때 | `~/문서/현장/260909` | 현장에서 손으로 패치하는 `application.yaml` 사본 | 패치본과 배포본의 값이 같은지 |
...
**대상이 repo 가 아니어도 된다.** 현장에서 손으로 패치하는 산출물 폴더처럼 git 밖에 있는 사본도
따라가야 하는 곳이다. `downstream-scan.sh` 는 repo 만 훑으므로 이런 것은 사람이 알려줘야 나온다.
알려주면 그때 기록한다.
