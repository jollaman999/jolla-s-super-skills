---
name: release-cut
description: 태그를 찍고 릴리즈 노트를 쓰고 바이너리를 올린 뒤, 이 릴리즈를 따라가야 할 다른 repo 가 어디인지 보고하는 데까지 간다. TRIGGER - "릴리즈 찍어줘", "태그 찍고 릴리즈", "vX.Y.Z 로 올려", "릴리즈 때리고 배포해". SKIP - 노드에 반영만 필요할 때(deploy-verify), 커밋·푸시만 할 때.
---
...
**맨 앞에 판정 블록을 둔다.** 사용자가 시킨 단계(태그·릴리즈·에셋·연동 repo·배포)마다 한 줄씩
`됨` / `안 됨` / `확인 필요` 로 적는다. 시켰는데 안 한 단계도 `안 함` 과 이유를 적는다.
배포까지 이어졌으면 `deploy-verify` 의 판정 줄을 이 블록에 합쳐 하나로 낸다 → `../shared/references/reporting.md` 6절
