---
name: release-cut
description: 태그를 찍고 릴리즈 노트를 쓰고 바이너리를 올린 뒤, CD 가 만든 이미지를 연동 repo 에 반영하는 데까지 간다. 버전 bump → 커밋 → 태그 → 릴리즈 노트 → 바이너리 첨부 → CD 대기 → 연동 repo 버전 갱신 순서로 게이트를 걸어 진행한다. TRIGGER - "릴리즈 찍어줘", "태그 찍고 릴리즈", "vX.Y.Z 로 올려", "릴리즈 때리고 배포해". SKIP - 노드에 반영만 필요할 때(deploy-verify), 커밋·푸시만 할 때.
---

# release-cut

버전 bump → 커밋 → 태그 → **릴리즈 노트** → 바이너리 첨부 → CD 이미지 대기 → **연동 repo 반영** → 배포.
...
버전이 코드 안에도 박혀 있다. 태그만 찍으면 실행 파일은 옛 버전을 말한다.
...
실제로 새 버전이 도는지는 `verify-impl` 이 본다.
