---
name: verify-impl
description: 소스에 구현된 기능을 파악하고, 실제 노드에 SSH로 붙거나 로컬에 띄워서 실증 검증한다. 체크리스트를 먼저 뽑아 민감 항목을 짚어 승인받은 뒤에만 실행한다. TRIGGER - "분석해줘", "분석해봐", "검증해줘", "제대로 동작하는지 확인", "확인해봐", "구현 확인", "동작 검증", "왜 안되는지 확인", "접속해서 확인", 또는 구현/배포 후 실제 동작 확인이 필요할 때, 또는 구현된 기능을 훑어 정리해 달라고 할 때. 애매하면 부른다 - 안 부르는 쪽이 예외다. SKIP - 버그 헌팅/코드 리뷰만 원할 때(/code-review), 코드를 새로 작성하거나 고치는 작업일 때, 파일을 열 필요도 없이 한 줄로 답이 끝나는 질문일 때.
---
