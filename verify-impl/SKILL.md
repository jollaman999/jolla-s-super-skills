---
name: verify-impl
description: 소스에 구현된 기능을 파악하고, 실제 노드에 SSH로 붙거나 로컬에 띄워서 실증 검증한다. TRIGGER - "분석해줘", "분석해봐", "검증해줘", "제대로 동작하는지 확인", "확인해봐", "구현 확인", "동작 검증", "왜 안되는지 확인", "접속해서 확인", 또는 구현/배포 후 실제 동작 확인이 필요할 때, 또는 구현된 기능을 훑어 정리해 달라고 할 때. 애매하면 부른다 - 안 부르는 쪽이 예외다. SKIP - **변경분**(이 PR·방금 고친 것·이 브랜치)에 대한 버그 헌팅·코드 리뷰일 때(/code-review), **무엇을 고칠지가 지목 안 된 개선 요청일 때**(design-first 의 survey 갈래가 받고, 거기서 이 skill 의 static 경로를 불러 쓴다), 무엇을 어디에 쓸지 이미 정해져 손만 대면 되는 작업일 때, 파일을 열 필요도 없이 한 줄로 답이 끝나는 질문일 때. 고치는 작업이어도 무엇을 고칠지 파악하는 단계는 이 skill 로 온다.
---
...
5. **비밀번호·토큰은 세션 안에서만 기억한다.** 파일에 쓰지 않고, 출력에서는 마스킹한다.
   **답변 텍스트에도 값을 되풀이하지 않는다** - 파일에 안 썼다는 설명 안에 값을 쓰는 것이 이 규칙의 대표 위반이다 (B′).
...
**P 가 `light` 면 A·B → 즉석 검증 → 보고 로 끝낸다.**
**P 가 `static` 이면 A·B → X → 보고 로 끝낸다.** 실행을 안 하므로 B′·D·E·F 를 건너뛴다.
판정표 → `references/routing.md`
...
flowchart TD
  ...
  N -->|새로| P{{"P 경로 판정"}}
  P -->|static| SA["A·B 탐색"] --> SX["X 스펙대조"] --> SG["보고 (전 항목 정적)"] --> G
...
대상: <repo> · 경로: full · 환경: 사이트A(192.0.2.11)
...
| F 검증 | **진행중** | 192.0.2.11 완료 / 192.0.2.12 미완 |
| H 원인추적 | 대기 | |

## 항목별
| id | 192.0.2.11 | 192.0.2.12 |
|----|-----------|-----------|
| VF-01 | pass | - |

## 다음 할 일
192.0.2.12 의 VF-01~12 를 verify-host 로 실행
```
...
**먼저 `static` 인지 본다.** 요청이 **읽고 정리해 달라는 것**이면 `static` 이다.
"소스 분석해줘", "구조 파악해줘", "어떻게 돼 있는지 정리해줘" 처럼 지금 무엇이 있는지를 묻고,
**맞게 도는지 판정해 달라는 말이 없는** 경우다. 붙을 생각이 없는데 접속 승인을 묻는 것이 낭비다.

| 요청이 바라는 것 | 경로 |
|------------------|------|
| 무엇이 있는지 알기 (읽고 정리) | `static` |
| 맞게 도는지 판정 (짧은 단발) | `light` |
| 맞게 도는지 판정 (전체·다중·배포 동반) | `full` |
...
`static` 경로: A·B 동시 → X 스펙대조 → 보고. **접속 승인(B′)과 체크리스트 승인(E)을 건너뛴다.**
실행도 접속도 안 하므로 승인받을 것이 없다. 대신 **보고의 모든 판정에 `(정적)` 을 붙이고,
실측해야 갈리는 것은 `unknown` 으로 남긴다.** 코드를 읽은 것을 동작 확인으로 쓰지 않는다.

`light` 경로: A·B 동시 → 대상 1~3개 즉석 검증 → 보고. **E 승인은 생략하되 B′ 실접속 승인은 생략하지 않는다.**

**승격은 한 방향이다.** `static` 중에 실행이나 접속이 필요해지면 `light` 나 `full` 로 올리고 알린다.
`light` 중 상태 변경이 필요해지거나 항목이 5개를 넘으면 `full` 로 올린다. 반대로 내려오지는 않는다.

판정표와 승격 규칙 → `references/routing.md`

### `design-first` 가 부른 경우

설계를 정하려고 **지금 무엇이 어떤지**를 물어온 것이다. 넘어온 질문이 곧 범위다.

| 지킬 것 | 왜 |
|---------|-----|
| 넘어온 질문에만 답한다 | 전수 검증을 하러 온 자리가 아니다 |
| 기본은 `static`, 실행이 필요하면 `light` | 질문 하나에 full 을 태우면 설계가 멈춘다 |
| 무엇을 만들지 정하지 않는다 | 그건 `design-first` 몫이다. 사실만 올린다 |
| 실접속 승인(B′)은 그대로 받는다 | 부른 주체가 skill 이어도 사람에게 묻는다 |

답은 `pass/fail/unknown` 과 근거로 낸다. "그래서 이렇게 만드시죠" 는 붙이지 않는다.
...
- **값을 쓰지 않는 곳에 답변 텍스트도 포함된다.** 파일·명령·evidence·보고서만이 아니라 **사용자에게 보내는 글 전부**다.
  언급해야 할 때는 `제공된 비밀번호`, `<마스킹>` 처럼 **지칭만** 한다.

**안 썼다고 말하면서 값을 쓰기.** 이 실패는 파일이 아니라 답변에서 난다.
파일에는 정말 안 적어 놓고, 그것을 설명하는 문장 안에 평문을 그대로 넣는다.

> 내가 쓴 것: "비밀번호는 파일에 쓰지 않았습니다. `<평문값>` 은 세션 컨텍스트에만 두고 다음 세션에는 다시 여쭙겠습니다"

앞 문장이 지켰다고 말한 규칙을 뒷 문장이 깬다. **값이 남았는지 보는 자리는 파일이 아니라 내가 지금 보내려는 글이다.**
"안 썼습니다", "기록하지 않았습니다" 를 쓰려는 순간이 가장 위험하다 - 그 문장에 값을 붙이지 않는다.
...
> 아래는 신경 쓰셔야 할 항목입니다:
> - VF-05는 `192.0.2.11`(운영)에서 **서비스를 재기동**합니다. 진행할까요?
...
- 비밀번호·토큰을 파일에 기록하거나 출력·**답변 텍스트**에 노출
  (**"파일에 안 썼다" 는 설명 안에 값을 쓰는 것도 노출이다**)
- 범위 밖 수정 (발견해도 보고만)
- `timeout` 없는 원격 명령
